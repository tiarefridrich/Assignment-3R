...
We are analyzing the surveys dataset to look at the relationship between different variables. 
...
#Plots
###1. Number of individuals of each sex by species surveyed
````{r number_individuals_species_sex}
ggplot(surveys_NAsrem) + stat_count(aes(x = species_id, fill = sex), position = "dodge")
````
Figure 1: Number of individuals of each sex by species surveyed.

The species DM has the highest population counted across the whole period for both male and female. 

###2. Number of individuals seen over time, separated by sex 
...
````
Figure 2: Number of individuals of each sex seen over time, separated by species 

The species PL has abruptly gone up since 1995 which had the minimum population before 1995.

###3. Number of individuals total of all the species surveyed each year by plot type
...
Plot type 'Control' has the highest record of individual total of all the species recorded.
...
The species 'DS' has the highest hindfoot length among others. 
...
```
Figure 5: Hindfoot length for male and female. The red dot indicates the mean (average) hindfoot length. 

Mean and median both the hindfoot length value for male are higher than that of female. 

###6. Males and females average weight yearly
...
ggplot(spe_meanweight, aes(x=year, y=mean_weight, color=sex)) +
  ...
  theme(axis.text.x = element_text(colour = "grey20", size = 10, angle = 90, hjust = 0.5, vjust = 0.5))+xlab('Year')+ylab('Average weight (gram)')
```
Figure 6: time series faceting plot showing yearly average weight of each male and female species.
Species PE has the highest average weight for both male and female.
...
Here, the p-value is less than 0.05, therefore the correlation coefficient (+0.14) is statistically sigificant though there is not a strong relationship between the weight and hindfoot length of OT species. 
