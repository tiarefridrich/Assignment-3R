...
Analyzing the surveys dataset to look for trends. Removed the NAs, which left only the rodent taxa. 
...
###2. Count of species observed
```{r bar_chart_individuals_each_species}
ggplot(surveys_NAsrem) + stat_count(aes(x = species_id, fill = species_id))+ylab('Number of individuals recorded')+xlab('Species ID')+theme(axis.text.x = element_text(angle = 90)) 
```
...
###4. Relationship between hindfoot length and weight of species OT
...
````
Figure 4: Relationship between hindfoot length and weight of individuals for species OT. 
...
###6. Number of species found by year in each plot type  
```{r species_count}
...
```
Figure 6: Time series faceting plot showing the count of each species yearly.

###7. Number of individuals surveyed each year by plot type
...
```
Figure 7: Time series faceting plot showing the count of individuals observed in each plot type yearly.

###8. Males and females average weight yearly
...
```
Figure 8: time series faceting plot showing yearly average weight of each male and female species.
...
###11. Number of individuals seen over time, separated by sex 
...
````
Figure 11: Number of individuals of each sex seen over time, separated by species 
